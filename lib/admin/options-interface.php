...
function optionsframework_help_tab () {
    ...
    $screen->add_help_tab( array(
        'id'	=> 'my_help_tab',
        'title'	=> __( 'My Help Tab', 'icy' ),
        'content'	=> '<p>' . __( 'Descriptive content that will show in My Help Tab-body goes here.', 'icy' ) . '</p>',
    ) );
}


function optionsframework_upgrade_info() {
	$checkout_url = 'http://wildweblab.com/?go=checkout&theme=icy';

	?>
	<div id="wwl-upgrade-box">
		<i class="icon-lightbulb"></i>
		<h1>This Theme Can Be Even Better!</h1>
		<div id="wwl-upgrade-content">
			Thank you for choosing Icy. Upgrade to PRO version and get access to <b>premium suppport</b> 
			and many useful features like: <b>portfolio</b> &amp; <b>grid templates</b>, 3 additional <b>color skins</b>, 
			responsive <b>slider</b> &amp; <b>related posts</b> that are nicely integrated, 
			<b>infinite scroll</b> (auto loading posts) and <b>more theme options</b>.
		</div>
		<a href="<?php echo $checkout_url; ?>" id="wwl-upgrade-btn" target="_blank">Upgrade Now</a>
		<div class="clear"></div>
	</div>
	<?php
}
...
function optionsframework_themeinfo() {
	$theme = wp_get_theme(get_template());

	printf('<h1 id="wwl-theme-name">%s</h1> <span id="wwl-theme-version">v%s</span>', 
		$theme->Name,
		$theme->Version
	);
}
...
function optionsframework_tabs() {
	...
	foreach ( $options as $value ) {
		...
		if ( $value['type'] == "heading" ) {
			...
			$menu .= '<li><a id="'.  esc_attr( $jquery_click_hook ) . '-tab" href="' . esc_attr( '#'.  $jquery_click_hook ) . '"><i class="' . ( ( isset( $value['icon'] ) ) ? esc_html( $value['icon'] ) : '' ) . '"></i>' . esc_html( $value['name'] ) . '</a></li>';
		}
	}

	$menu .= '<li><a id="of-option-importexport-tab" href="#of-option-importexport"><i class="icon-floppy"></i>Import / Export</a></li>';
...
function optionsframework_importexport() {
	?>
	<div class="group" id="of-option-importexport">
		<div id="section-export_settings" class="section section-textarea">
			<h4 class="heading"><?php _e( 'Export Settings', 'icy' ); ?></h4>
			<div class="option">
				<div class="controls">
					<input type="submit" class="button-secondary wwl-export-button" name="export_settings" value="<?php esc_attr_e( 'Download Settings File', 'icy' ); ?>" />
				</div>
				<div class="explain"></div>
			</div>
		</div>

		<div id="section-import_settings" class="section section-textarea">
			<h4 class="heading"><?php _e( 'Import Settings', 'icy' ); ?></h4>
			<div class="option">
				<div class="controls">
					<input type="file" name="file" class="wwl-import-file" />
					<input type="submit" class="button-secondary wwl-import-button" name="import_settings" value="<?php esc_attr_e( 'Upload Settings', 'icy' ); ?>" onclick="return confirm( '<?php print esc_js( __( 'Click OK to upload your settings. Any current theme settings will be lost!', 'icy' ) ); ?> ');" />
				</div>
				<div class="explain"></div>
			</div>
		</div>
